    ...
    for k = 1:Reg.n
        ...
        if bc{k}.NumS > 0
            ...
            for p = 1:bc{k}.NumS
                ...
                Eincp = bc{k}.s(p).fct(yC{k},i,t-ds/c_c/2,bc{k}.s(p).paras);
                
                Hy{k}(bc{k}.s(p).xpos,:) = Hy{k}(bc{k}.s(p).xpos,:) + dt/ds/c_mu_0*Einc;
                Ez{k}(bc{k}.s(p).xpos-1,:) = Ez{k}(bc{k}.s(p).xpos-1,:) - dt/ds/c_eps_0/c_eta_0*Eincp;
            end
        end
